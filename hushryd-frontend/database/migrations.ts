...
export const createInitialSchemaMigration = (dbType: DatabaseType): Migration => {
  ...
  return {
    ...
    up: async (connection: DatabaseConnection) => {
      ...
      const tableOrder = [
        'users', 'emergency_contacts', 'rides', 'bookings', 'sos_alerts', 'admins',
        'transactions', 'payments', 'wallets', 'wallet_transactions', 'reviews',
        'driver_profiles', 'bank_accounts', 'routes', 'notifications', 'support_tickets',
        'fares', 'commissions', 'payouts', 'promo_codes', 'user_promo_usages'
      ];
      
      for (const tableName of tableOrder) {
        if (tableName in schema && schema[tableName as keyof typeof schema]) {
          await connection.query(schema[tableName as keyof typeof schema]);
        }
      }
    },
    down: async (connection: DatabaseConnection) => {
      ...
      const tableOrder = [
        'user_promo_usages', 'promo_codes', 'payouts', 'commissions', 'fares',
        'support_tickets', 'notifications', 'routes', 'bank_accounts', 'driver_profiles',
        'reviews', 'wallet_transactions', 'wallets', 'payments', 'transactions',
        'admins', 'sos_alerts', 'bookings', 'rides', 'emergency_contacts', 'users'
      ];
